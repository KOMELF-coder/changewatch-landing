...
try{
 ...
 for(mode of ['success','http','network','timeout']){await fill();const before=formCount,resets=await page.evaluate(()=>resetCount);
 // Trigger the duplicate in the same browser task, before even an immediate network failure settles.
 const sending=await page.locator('[type=submit]').evaluate(button=>{
  button.click();button.form.dispatchEvent(new Event('submit',{cancelable:true,bubbles:true}));
  return {disabled:button.disabled,busy:button.form.getAttribute('aria-busy'),button:button.textContent,status:document.querySelector('#form-status').textContent};
 });
 assert.deepEqual(sending,{disabled:true,busy:'true',button:'Sending…',status:'Sending your request…'});
