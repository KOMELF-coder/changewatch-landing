...
for(const [i,route] of routes.entries()){
 ...
 assert.equal(post.wordCount,words);assert.equal(post.timeRequired,'PT'+Math.ceil(words/200)+'M');assert.equal(post.headline,await p.locator('h1').textContent());assert.equal(post.mainEntityOfPage['@id'],pub+route);assert.equal(post.datePublished,'2026-09-24');assert.equal(post.dateModified,'2026-09-24');assert.equal(await p.locator('meta[property="article:published_time"]').getAttribute('content'),'2026-09-24');assert.equal(post.image,await p.locator('meta[property="og:image"]').getAttribute('content'));assert.ok(!post.translationOfWork);assert.ok(!/Brouillon|Draft/.test(await p.locator('main').textContent()));
 ...
 console.log('PASS '+lang+': independent metadata/schema and publication dates, arithmetic/resources, anchors, localized CSV, six widths, axe320/1440, keyboard, 200% text/layout, language switch, consent/route privacy (mock GA)');
}
