...
const routes=['/blog/analyser-la-concurrence/','/en/blog/competitor-price-analysis/'];
const shots=process.env.CW_SCREENSHOT_DIR||path.join(process.env.TEMP||'/tmp','cw-analysis-shots');
...
for(const [i,route] of routes.entries()){
 ...
 assert.equal(post.wordCount,words);assert.equal(post.timeRequired,'PT'+Math.ceil(words/200)+'M');assert.equal(post.headline,await p.locator('h1').textContent());assert.equal(post.mainEntityOfPage['@id'],pub+route);assert.ok(!('datePublished' in post));assert.ok(!('dateModified' in post));assert.equal(await p.locator('meta[property="article:published_time"]').count(),0);assert.equal(post.image,await p.locator('meta[property="og:image"]').getAttribute('content'));if(i)assert.equal(post.translationOfWork['@id'],pub+routes[0]+'#article');
 ...
 const rows=await p.locator('.analysis-data tbody tr').evaluateAll(es=>es.map(e=>({sku:e.dataset.sku,price:+e.dataset.price,discount:+e.dataset.discount,shipping:+e.dataset.shipping,total:+e.dataset.total,displayed:e.querySelector('td:nth-child(6)').textContent})));assert.equal(rows.length,8);
 ...
 for(const width of [320,375,390,768,1024,1440]){
  ...
  if([375,390,1440].includes(width)){await p.screenshot({path:path.join(shots,lang+'-'+width+'.png'),fullPage:true});await p.screenshot({path:path.join(shots,lang+'-'+width+'-header.png')});await p.locator('.analysis-figure').screenshot({path:path.join(shots,lang+'-'+width+'-diagram.png')});}
 ...
 console.log('PASS '+lang+': metadata/schema without publication dates, arithmetic for 8 rows/2 medians/gaps, anchors, CSV29, six widths, axe320/1440, keyboard, 200% text/layout, language switch, consent/route privacy (mock GA)');
}
